...
export default async function EditAssessmentPage({ params }: { params: Promise<{ assessmentId: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link href={`/admin/assessments/${assessment.id}`} className="focus-ring inline-flex rounded-lg border border-brand-border bg-brand-surface px-4 py-2 text-sm font-semibold hover:border-brand-primary hover:bg-brand-primary hover:text-white">Back to assessment</Link>
