...
export default function AdminGroupOverview({ groups, studentsPerGroup }: { groups: Group[]; studentsPerGroup: number }) {
  return (
    <section className="mx-auto w-full max-w-3xl">
      <div className="mb-5"><h2 className="text-2xl font-bold">Groups</h2><p className="mt-1 text-sm text-brand-muted">Review group membership and educator ownership.</p></div>
      {groups.length === 0 ? <div className="rounded-xl border border-dashed border-brand-border bg-brand-surface p-10 text-center shadow-soft"><h3 className="font-bold">No groups created</h3><p className="mt-2 text-sm text-brand-muted">Groups will appear here after an educator creates them.</p></div> : (
